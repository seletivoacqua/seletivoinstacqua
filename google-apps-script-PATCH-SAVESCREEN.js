...
function handleRequest(e) {
  try {
    ...
    // ✅ CORREÇÃO: saveScreening retorna estrutura completa {success, message, ...}
    // Não deve ser envolvido em { success: true, data: ... }
    if (action === 'saveScreening') {
      try {
        const result = saveScreening(params);
        Logger.log('✅ saveScreening resultado: ' + JSON.stringify(result).substring(0, 200));
        return createCorsResponse(result); // ← Retorna direto
      } catch (actionError) {
        Logger.log('❌ Erro em saveScreening: ' + actionError.toString());
        return createCorsResponse({
          success: false,
